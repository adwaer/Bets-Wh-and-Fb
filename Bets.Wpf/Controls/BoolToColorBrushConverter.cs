        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (parameter != null)
            {
                ...
                if (!string.IsNullOrEmpty(parameterstring))
                {
                    ...
                    if (count > 3)
                    {
                        if (int.TryParse(parameters[3], out valComparator))
                        {
                        }
                    }
                }
            ...
            var diff = (int)value;
            if (diff == valComparator)
            {
                color = new SolidColorBrush(colorIfTrue) { Opacity = opacity };
            }
            else if (diff == -valComparator)
            {
                color = new SolidColorBrush(Colors.OrangeRed) { Opacity = opacity };
            }
            else
            {
